...
router.get("/:id", authenticateJWT, async (req, res) => {
  ...
  console.log("Mypageendpoint", userId);
  ...
  try {
    ...
    let coupleDiary = [];
    if (coupleId) {
      const [coupleDiaryResult] = await db.query(
        `SELECT feeling, COUNT(*) AS count
         FROM diarytable
         WHERE user_id = ?
           AND privacy = 'Couple'
    ...
    if (coupleId) {
      ...
      coupleMonth = coupleMonthCount.length > 0 ? coupleMonthCount[0].count : 0;
      coupleAll = coupleAllCount.length > 0 ? coupleAllCount[0].count : 0;
    }
...
try {
    ...
    [updateResult2] = await db.query(
        `UPDATE DiaryDB.users SET date = ? WHERE nickname = ?`,
